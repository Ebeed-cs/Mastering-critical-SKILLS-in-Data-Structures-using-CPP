...
private:
    ...
    BinarySearchTree* problem1(int target, BinarySearchTree* node) {
        if (!node)
            return nullptr;

        if (target < node->data)
            node->left = problem1(target, node->left);
        else if (target > node->data)
            node->right = problem1(target, node->right);
        else {
            if (!node->left && !node->right) {
                ...
                node->left = problem1(node->data, node->left);
            }
        }
        return node;
    }
