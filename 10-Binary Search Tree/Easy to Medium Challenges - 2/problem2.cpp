...
private:
    ...
    BinarySearchTree* problem2(int target, BinarySearchTree* node) {
        if (!node)
            return nullptr;

        if (target < node->data)
            node->left = problem2(target, node->left);
        else if (target > node->data)
            node->right = problem2(target, node->right);
        else {
