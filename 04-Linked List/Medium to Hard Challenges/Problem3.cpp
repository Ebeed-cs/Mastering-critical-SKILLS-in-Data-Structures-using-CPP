...
public:
    ...
    void Problem3(LinkedList& another) {
        ...
        while (my_cur || his_cur) {
            ...
            if (my_cur) {
                ...
                insert_end(my_value);
            // we can even stop earlier and add the rest nodes in directly to our nodes "use old nodes"
        }
        if (carry)
            insert_end(carry); // if there is a remaning carry not added yet 
...
void test5() {
    ...
    string expected = "7 4 9 3 4 3 6 9 2 3";
    string result = list1.debug_to_string();
    if (expected != result) {
        cout << "no match:\nExpected: " << expected << "\nResult  : " << result << "\n";
        assert(false);
