...
public:
    ...
    void Rem2(int key) {  // O(n) time & o(1) memory
        if (!length) return;
        length--;
        Node* prev = head;
        for (Node* cur = head; cur->next; cur = cur->next)
            if (cur->next->data == key)
                prev = cur;
        if (prev == head && prev->data == key) { // is the  head node is the target ?
            if (head == tail) tail = tail->next;
            head = head->next;
            delete prev;
        }
        else if (prev->next->data == key) // is the next node the target ?
            delete_next_node(prev);
        length++; // then the node key not found -> then reset the length to it's orginal value
    }
