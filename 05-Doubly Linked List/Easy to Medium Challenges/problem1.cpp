...
public:
    ...
    Node* delete_and_link(Node* cur) {  // returns the previous node to the delted one
    ...
    // CLOSE TO THE FUNCTION "delete_node_with_key()"
    void problem1(int value) { // o(n) time , o(1) memory
        if (!length)
            return;
        for (Node* cur = head; cur; cur = cur->next) {
            if (head->data == value) {
