...
public:
    ...
    void swap_f_b(int k) { // o(n) time , o(1) memory

        if (length <= 1 || k > length) return;
        Node* cur1 = head;
        Node* cur2 = tail;

        bool is_swap = k > length / 2;
        while (--k > 0) {
            cur1 = cur1->next;
            cur2 = cur2->prev;
        }

        if (is_swap) std::swap(cur1, cur2);
        bool is_head = cur1 == head;

        Node* temp1 = cur1->prev;
        Node* temp2 = cur2->prev;

        cur2->prev = temp1;
        cur1->prev = temp2;

        if (!is_head)temp1->next = cur2;
        temp2->next = cur1;
        ...
        temp1->prev = cur2;
        if (!is_head) temp2->prev = cur1;

        if (is_head) std::swap(head, tail);
    }
    ...
    void problem3() {
        ...
        if (length % 2 != 0) {
            insert_front(-1234); // convert from odd list to even "dummy value"
            add_dummy = true;
        }
        ...
        int count = length / 2;
        while (count-- > 0) {
            Node* fnode = cur1->next;
            Node* snode = cur2->prev;
            bool is_head = cur1 == head;
