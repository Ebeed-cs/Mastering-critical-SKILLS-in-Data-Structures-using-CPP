...
public:
    ...
    problem4() { tree = new AVLTree(); }

    void enqueue(int val, int k) {
        tree->insert_value(val, k);
    }

    int dequeue() {
        BinaryNode* del_node = tree->max_tree();
        int res = del_node->values.front();
        del_node->values.pop();
        if (del_node->values.empty())
            tree->delete_value(del_node->key);
        return res;
    }
