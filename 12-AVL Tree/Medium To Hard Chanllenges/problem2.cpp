...
private:
	...
	BinaryNode* problem2(int target, BinaryNode* node) {
		if (!node)
			return nullptr;

		if (target < node->data) {	// I can be an upper bound
			// Can we find tighter answer in the left?
			BinaryNode* ans = problem2(target, node->left);
			if (ans)
				return ans;
			return node;
		}
		return problem2(target, node->right);
	}
...
public:
	...
	pair<bool, int> problem2(int target) {
		if (root) {
			BinaryNode* res = problem2(target, root);
			if(!res)
...
void test_upper() {
	...
	sort(v.begin(), v.end());
	for (int i = 0; i < v.size(); ++i)
		cout << v[i]<< " " << tree.problem2(v[i] ).second << "\n";
}
