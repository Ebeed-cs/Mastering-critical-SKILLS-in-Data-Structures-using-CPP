...
private:
    ...
    BinaryNode* problem1(int target, BinaryNode* node) {
        if (!node)
            return nullptr;

        if (target <= node->data) {	// I can be a lower bound
            // Can we find tighter answer in the left?
            BinaryNode* ans = problem1(target, node->left);
            if (ans)
                return ans;
            return node;
        }
        return problem1(target, node->right);
    }
...
public:
    ...
    pair<bool, int> problem1(int target) {
        if (root) {
            BinaryNode* res = problem1(target, root);
            if (!res)
...
void test_lower() {
    ...
    for (int i = 0; i < v.size(); ++i)
        cout << v[i] + 1 << " " << tree.problem1(v[i] + 1).second << "\n";
}
